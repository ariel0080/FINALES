...
char* Array_sin_rep(char* palabra)

{
    char letra;

    char* arreglo_letras;
    arreglo_letras=malloc(sizeof(char)*100);

    arreglo_letras[0]=palabra[0];//printf("Primera letra copiada: %c\n\n",palabra[0]);
    arreglo_letras[1]='\0';

    for(int i=1;i<strlen(palabra);i++) //no include \0
        {
            int* p = strchr(arreglo_letras,palabra[i]);
            if(p==NULL)
            {
                int longi = strlen(arreglo_letras);//printf("longitud: %d\n",longi);
                arreglo_letras[longi]=palabra[i];//printf("letra a agregar: %c\n",palabra[i]);
                arreglo_letras[longi+1]='\0';//system("pause");
            }
...
int fLetra(void* pElement,void* letra) //
{
    int returnaux = -1;
    if(pElement!=NULL)
    {
        eLetra *tmp =(eLetra*)pElement;
        letra = *((char*)letra);

        char l;
        l=get_eLetra_letra(tmp);//printf("tomo letra: %c--",letra);

        if(letra==l)
        {
            returnaux =0;printf("deberia agregar");
        }
        else
        {
            returnaux =1;
